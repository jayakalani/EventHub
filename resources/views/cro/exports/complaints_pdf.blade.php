...
<body>
    ...
    <div class="header">
        ...
        <p class="meta">
            ...
            · {{ number_format($complaints->count()) }} {{ $complaints->count() === 1 ? 'complaint' : 'complaints' }}
            @if ($filterBits !== [])
                · {{ implode(' · ', $filterBits) }}
            @endif
        </p>
    ...
    <table class="data">
        <thead>
            <tr>
                ...
                <th class="right" style="width: 70px;">Files</th>
        ...
        <tbody>
            @forelse ($complaints as $complaint)
                <tr>
                    ...
                    <td>{{ $complaint->assignee?->full_name ?? 'Unassigned' }}</td>
                    <td>{{ $complaint->status->label() }}</td>
                    <td class="right">{{ $complaint->attachments->count() }}</td>
                    <td>{{ $complaint->created_at?->format('d M Y, H:i') ?? '—' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" class="empty">No complaints match the selected filters.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="footer">
        EventHub · Confidential CRO complaints export · {{ config('app.name', 'EventHub') }}
    </div>
</body>
</html>
