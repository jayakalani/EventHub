            ...
            <div class="border border-dark border-2 rounded-2 p-4">
                    ...
                    <div class="col-md-6">
                        <select class="form-control" id="hosted_by" name="hosted_by" required>
                            ...
                            @foreach ($hosts as $host)
                                <option value="{{ $host->id }}" {{ $event->hosted_by == $host->id ? 'selected' : '' }}>
                                    {{ $host->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('hosted_by')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <select class="form-control" id="category_id" name="category_id" required>
                            ...
                            @foreach ($event_categories as $event_category)
                                <option value="{{ $event_category->id }}" {{ $event->category_id == $event_category->id ? 'selected' : '' }}>
                                    {{ $event_category->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        ...
                        @error('time')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        @error('place')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <input id="no_of_seats" type="text" class="form-control @error('no_of_seats') is-invalid @enderror"
                               name="no_of_seats" value="{{ old('no_of_seats', $event->no_of_seats) }}" required>
                        @error('no_of_seats')<span class="invalid-feedback"><strong>{{ $message }}</strong></span>@enderror
                    </div>
                </div>
                ...
                <!-- CRO -->
                <div class="row mb-3">
                    <label for="contact_person" class="col-md-4 col-form-label text-md-end">Customer Relations Officer:</label>
                    <div class="col-md-6">
                        <select class="form-control" id="contact_person" name="contact_person" required>
                            <option value="">Select Contact Person</option>
                            @foreach ($croUsers as $user)
                                <option value="{{ $user->id }}" {{ $event->contact_person == $user->id ? 'selected' : '' }}>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <input type="file" id="cover" name="cover" class="form-control-file" accept=".jpg,.jpeg,.png">
                        <small class="text-muted">Accepted file type: JPG, JPEG, PNG | must be less than 2MB</small>
                        @if($event->cover)
                            <div class="mt-2">
                                <img src="{{ asset('storage/'.$event->cover) }}" alt="Current Cover" class="w-32 h-20 object-cover rounded">
                            </div>
                        @endif
                    </div>
                </div>
