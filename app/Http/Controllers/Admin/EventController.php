...
{
    /**
     * Platform-wide event directory for admins.
     */
    public function index(Request $request): View
    {
        ...
        $stats = [
            ...
            'unpublished' => (clone $query)->where('status', Event::STATUS_UNPUBLISHED)->count(),
            'postponed' => (clone $query)->where('status', Event::STATUS_POSTPONED)->count(),
            'cancelled' => (clone $query)->where('status', Event::STATUS_CANCELLED)->count(),
        ];
        ...
        $organizers = User::query()
            ->whereHas('userRole', fn (Builder $roleQuery) => $roleQuery->where('name_en', UserRole::ORGANIZER))
            ->orderBy('first_name')
            ->orderBy('last_name')
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('admin.exports.events_pdf', compact('events'))
