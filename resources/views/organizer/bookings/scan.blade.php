        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-0.5 text-sm text-slate-500">
                    {{ $guestList['scan_subtitle'] }}
                </p>
            </div>

            <a href="{{ route($guestList['index'], array_filter(['event_id' => $eventId])) }}"
            ...
            @if ($events->isEmpty())
                <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
                    ...
                    <p class="mt-1 text-amber-800">
                        {{ $guestList['scan_empty'] }}
                    </p>
                    <a href="{{ route($guestList['events_index'] ?? $guestList['home']) }}"
                        ...
                        {{ $guestList['scan_empty_cta'] }}
                    </a>
                </div>
            @else
                <div class="space-y-4">
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <form method="POST" action="{{ route($guestList['scan_submit']) }}" id="scan-form"
                            class="space-y-3">
                            @csrf
