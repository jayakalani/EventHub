@php
    ...
    $statusAccents = [
        'open' => ['top' => 'border-t-amber-500', 'iconBg' => 'bg-amber-100/70', 'iconText' => 'text-amber-600', 'value' => 'text-amber-700', 'icon' => 'bi-envelope-open'],
        ...
        <div class="mx-auto max-w-7xl space-y-5 px-4 sm:px-6 lg:px-8">
            ...
            <section class="glass-panel overflow-hidden !rounded-2xl">
                <div class="relative px-4 py-4 sm:px-6 sm:py-5">
                    ...
                    <div class="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0">
                            <p class="text-[11px] font-semibold uppercase tracking-wide text-indigo-600">Support queue</p>
                            <h1 class="mt-0.5 truncate text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">Inquiries</h1>
                            <p class="mt-1 text-sm text-slate-500">
                                Assigned events only · inquiries go to you as the event CRO, with no claim needed.
                            </p>
                        </div>
                        <div class="flex flex-wrap items-center gap-2">
                            <a href="{{ route('cro.inquiries.export.csv', $filterQuery) }}"
                            ...
                            <a href="{{ route('cro.inquiries.export.pdf', $filterQuery) }}"
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    @foreach ($statuses as $s)
                        ...
                        <a href="{{ route('cro.inquiries.index', array_filter(array_merge($filterQuery, ['status' => $s->value]))) }}"
                            class="glass-card kpi-lift group border-t-4 {{ $accent['top'] }} p-4 sm:p-5 {{ ($filters['status'] ?? '') === $s->value ? 'ring-1 ring-indigo-300/70 bg-white/80' : '' }}">
            ...
            <section class="glass-panel !rounded-2xl p-4 sm:p-5">
                <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h2 class="text-sm font-semibold text-slate-900">Filter inquiries</h2>
                ...
                <form method="GET" action="{{ route('cro.inquiries.index') }}"
                    ...
                    <div class="xl:col-span-3">
                        <label for="inquiry_q" class="mb-1.5 block text-xs font-semibold text-slate-600">Search</label>
                        <div class="relative">
                            <i class="bi bi-search pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-slate-400"></i>
                            <input id="inquiry_q" type="text" name="q" value="{{ $filters['q'] ?? '' }}"
                    ...
                    <div class="xl:col-span-2">
                        <label for="inquiry_status" class="mb-1.5 block text-xs font-semibold text-slate-600">Status</label>
                    ...
                    <div class="xl:col-span-3">
                        <label for="inquiry_event" class="mb-1.5 block text-xs font-semibold text-slate-600">Event</label>
                        <select id="inquiry_event" name="event"
                            class="w-full rounded-lg border border-white/70 bg-white/60 py-2 pl-3 pr-9 text-sm font-medium text-slate-700 shadow-sm backdrop-blur-md focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="xl:col-span-2">
                        <label for="inquiry_from" class="mb-1.5 block text-xs font-semibold text-slate-600">From</label>
                        <input id="inquiry_from" type="date" name="from" value="{{ $filters['from'] ?? '' }}"
                    ...
                    <div class="flex items-end gap-2 xl:col-span-2">
                        <div class="min-w-0 flex-1">
                            <label for="inquiry_to" class="mb-1.5 block text-xs font-semibold text-slate-600">To</label>
                            <input id="inquiry_to" type="date" name="to" value="{{ $filters['to'] ?? '' }}"
                                class="w-full rounded-lg border border-white/70 bg-white/60 px-3 py-2 text-sm text-slate-700 shadow-sm backdrop-blur-md focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <button type="submit"
            ...
            <section class="glass-card overflow-hidden !rounded-2xl !p-0 hover:!translate-y-0">
                <div class="flex flex-col gap-2 border-b border-white/60 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                    <div>
                        <h2 class="text-sm font-semibold text-slate-900">Inquiry list</h2>
                        <p class="text-xs text-slate-500">
                            Showing
                            <span class="font-semibold text-slate-700">{{ $inquiries->firstItem() ?? 0 }}</span>–
                            <span class="font-semibold text-slate-700">{{ $inquiries->lastItem() ?? 0 }}</span>
                            of
                            <span class="font-semibold text-slate-700">{{ number_format($inquiries->total()) }}</span>
                        </p>
                    </div>
                    <p class="text-xs font-medium text-slate-500">
                        Page {{ $inquiries->currentPage() }} of {{ max(1, $inquiries->lastPage()) }}
                    </p>
                </div>
                ...
                @if ($inquiries->isEmpty())
                    <div class="flex flex-col items-center justify-center px-4 py-16 text-center">
                        ...
                        <p class="text-sm font-semibold text-slate-800">No inquiries</p>
                        <p class="mt-1 max-w-sm text-xs text-slate-500">
                            No inquiries match the selected filters for your assigned events.
                        </p>
                    </div>
                @else
                    <div class="divide-y divide-white/50">
                        @foreach ($inquiries as $inquiry)
                            <a href="{{ route('cro.inquiries.show', $inquiry) }}"
                                ...
                                <div class="flex min-w-0 items-start gap-3">
                                    <span class="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50/90 text-indigo-600 ring-1 ring-indigo-100/80">
                                        <i class="bi bi-chat-left-text"></i>
                                    </span>
                                    <div class="min-w-0 space-y-1.5">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <h3 class="truncate text-sm font-bold text-slate-900">{{ $inquiry->subject }}</h3>
                                        ...
                                        <p class="line-clamp-2 text-sm text-slate-600">{{ $inquiry->message }}</p>
                                    </div>
                                </div>
                                <div class="flex shrink-0 items-center gap-3 self-end sm:self-center">
                                    <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $inquiry->status->badgeClass() }}">
