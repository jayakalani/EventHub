<x-app-layout>
    <div class="max-w-3xl mx-auto py-10">
        <div class="bg-white shadow-lg rounded-xl p-8">
            <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Edit Host</h2>
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('organizer.hosts.update', $host->id) }}"
                class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input id="name" type="text" name="name"
                        ...
                        value="{{ old('name', $host->name) }}" required autofocus>
                    @error('name')
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        value="{{ old('email', $host->email) }}" required>
                    @error('email')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="contact_number" type="text" name="contact_number"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        value="{{ old('contact_number', $host->contact_number) }}" required>
                ...
                <div>
                    ...
                    @if ($host->cover)
                        <div class="mt-3">
                            <img src="{{ asset('uploads/covers/hosts/' . $host->cover) }}" alt="Current Cover"
