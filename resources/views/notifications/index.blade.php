<x-app-layout>
    @php
        ...
        $accents = [
            ...
            'feedback' => ['tile' => 'bg-yellow-500/10 text-yellow-700', 'bar' => 'from-yellow-500 to-amber-300', 'pill' => 'bg-yellow-600 text-white'],
        ...
        $toneByType = [
            ...
            'inquiry_resolved' => 'bg-emerald-50 text-emerald-700 ring-emerald-100',
            'complaint_replied' => 'bg-rose-50 text-rose-700 ring-rose-100',
            'complaint_resolved' => 'bg-emerald-50 text-emerald-700 ring-emerald-100',
            'saved_event_published' => 'bg-pink-50 text-pink-700 ring-pink-100',
            'event_rating_nudge' => 'bg-yellow-50 text-yellow-800 ring-yellow-100',
            'event_review_digest' => 'bg-yellow-50 text-yellow-800 ring-yellow-100',
        ...
        <div class="relative mx-auto max-w-5xl space-y-5 px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 px-1">
                ...
                @if ($activeCategory)
                    ...
                    @if (! $isAdmin && ! $isCro && ! $isOrganizer && $activeCategory === \App\Enums\AttendeeNotificationCategory::Wishlist)
                        <a href="{{ route('attendee.saved.index') }}"
                            class="inline-flex items-center gap-1.5 text-xs font-semibold text-pink-600 transition hover:text-pink-700">
                            <i class="bi bi-bookmark-heart" aria-hidden="true"></i>
                            {{ t(['en' => 'View saved events', 'si' => 'සුරකින ලද ප්‍රසංග බලන්න']) }}
                        </a>
                    @endif
                @endif
            </div>
            ...
            @empty
                <div class="rounded-3xl border border-dashed border-slate-300/80 bg-white/60 p-12 text-center backdrop-blur-xl">
                    ...
                    @elseif (! $isAdmin && ! $isCro && ! $isOrganizer && $activeCategory === \App\Enums\AttendeeNotificationCategory::Wishlist)
                        <a href="{{ route('attendee.saved.index') }}"
                            class="mt-5 inline-flex items-center gap-2 rounded-xl bg-pink-600 px-4 py-2 text-sm font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-pink-700">
