        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                <a href="{{ route('admin.audit-logs.export.csv') }}"
                    class="inline-flex items-center px-3 py-1 bg-white border border-gray-300 text-sm text-gray-700 rounded hover:bg-gray-50">Export
                    CSV</a>
                <a href="{{ route('admin.audit-logs.export.pdf') }}"
                    class="inline-flex items-center px-3 py-1 bg-blue-600 text-sm text-white rounded hover:bg-blue-700">Export
                    PDF</a>
            </div>
        </div>
    </x-slot>
                ...
                <div class="border-t border-gray-200">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($logs as $log)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $log->user?->full_name ?? 'System' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $log->action }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $log->model_type }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $log->model_id }}
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500">No audit logs
                                        found.</td>
                                </tr>
                            @endforelse
                        </tbody>
