<x-app-layout>
    @php
        ...
        $showHandoff = $event->isPostponed() || $event->isCancelled();
    @endphp

    <div class="cro-event-show relative isolate overflow-hidden py-5 sm:py-6">
        ...
        <div class="mx-auto max-w-5xl space-y-5 px-4 sm:px-6 lg:px-8">
            <section class="glass-panel overflow-hidden !rounded-2xl">
                <div class="relative px-4 py-4 sm:px-6 sm:py-5">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-slate-600">Assigned event</p>
            ...
            <section class="flex flex-wrap gap-2">
                <a href="{{ route('cro.bookings.index', ['event_id' => $event->id]) }}"
                ...
                <a href="{{ route('cro.inquiries.index', ['event' => $event->id]) }}"
                    class="btn-smooth inline-flex items-center gap-1.5 rounded-lg border border-cyan-100 bg-cyan-50/80 px-3 py-2 text-xs font-semibold text-cyan-700 hover:bg-cyan-100">
                    <i class="bi bi-chat-dots"></i>
                    Inquiries
                </a>
                <a href="{{ route('cro.complaints.index', ['event' => $event->id]) }}"
                    class="btn-smooth inline-flex items-center gap-1.5 rounded-lg border border-rose-100 bg-rose-50/80 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-100">
                    <i class="bi bi-exclamation-triangle"></i>
                    Complaints
                </a>
                <a href="{{ route('cro.refund-requests.index', ['event' => $event->id]) }}"
                    class="btn-smooth inline-flex items-center gap-1.5 rounded-lg border border-amber-100 bg-amber-50/80 px-3 py-2 text-xs font-semibold text-amber-800 hover:bg-amber-100">
                    <i class="bi bi-arrow-counterclockwise"></i>
                    Refunds
                </a>
                @if ($showHandoff)
                    <a href="{{ route('cro.handoffs.show', $event) }}"
                        class="btn-smooth inline-flex items-center gap-1.5 rounded-lg border border-violet-100 bg-violet-50/80 px-3 py-2 text-xs font-semibold text-violet-700 hover:bg-violet-100">
            ...
            @if ($event->cancellation_reason || $event->postponement_reason)
                <section class="glass-card p-5">
                    <h2 class="text-sm font-semibold text-slate-900">Schedule notes</h2>
